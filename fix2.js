...
files.forEach(f => {
  ...
  // Check popup texts (Korean inside script block)
  text = text.replace(/장바구니에 담았어요./g, '장바구니에 담았어요.');
  text = text.replace(/CART 보기/g, 'CART 보기');
  text = text.replace(/계속 쇼핑하기/g, '계속 쇼핑하기');
  text = text.replace(/주문 완료!/g, '주문 완료!');
  text = text.replace(/주문 내역 보기/g, '주문 내역 보기');
  text = text.replace(/홈으로 돌아가기/g, '홈으로 돌아가기');
  text = text.replace(/담았어요./g, '담았어요.');

  // Detail / Cart / Checkout / Color pages have other texts!
  // Wait, I should also fix the headers in other pages!
  text = text.replace(/<button class="app__icon" aria-label="뒤로가기"/g, '<button class="app__icon" aria-label="뒤로가기"'); // In case it was broken

  // Oh, mypage has specific texts:
  // "주문 내역", "배송중", "배송완료", "Track Order" etc.
  // Did they break?
  // Let's run this first and then check if anything else is broken!
